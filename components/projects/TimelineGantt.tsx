      ...
      <div className="w-full min-w-[600px]">
        ...
        <div className="relative">
          ...
          {tasks.map((t, rowIdx) => {
            ...
            return (
              <div key={t.id} className="grid grid-cols-[240px_1fr] hover:bg-muted/30 transition-colors">
                <div className="px-4 py-3 text-[13px] font-medium text-foreground border-r border-border truncate flex items-center">
                  {t.name}
                </div>
                <div className="relative px-4 py-0">
                  <div className="grid h-full" style={{ gridTemplateColumns: `repeat(${days.length}, minmax(3rem, 1fr))` }}>
                    {Array.from({ length: days.length }).map((_, i) => (
                      <div key={i} className="border-l border-border/30 h-12 first:border-l-0" />
                    ))}
                  </div>
                  <div
                    className="absolute top-1/2 -translate-y-1/2 h-7 rounded-md bg-primary/20 border border-primary/40 px-2 flex items-center cursor-pointer hover:bg-primary/30 transition-colors"
